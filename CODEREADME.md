...
### 2026-06-16 — 歌单单曲更多选项完善与重构

- **单曲更多选项 BottomSheet (PlaylistScreen.kt)**：
  - 移除了原歌曲列表中外置的爱心图标，仅保留 `MoreVert` 图标，使列表项更为紧凑美观。
  - 为歌曲更多按钮集成了底部弹窗 BottomSheet。其设计参考播放器更多选项（`SongMoreOptionsSheet.kt`）的风格。
  - 提供选项：下一首播放（动态追加到播放队列）、喜欢/取消喜欢（基于单曲红心状态）、收藏到歌单（调起选择/新建歌单弹窗）、歌手跳转、专辑跳转（扩展了跨歌单/专辑页面的导航接口）。
  - 精简重复布局，抽取并封装了通用的 `@Composable private fun OptionRow` 组件，消除多处模板代码冗余。
- **状态维护与内存优化 (PlaylistViewModel.kt)**：
  - 新增 `toggleLikeSong(songId, isLike)` 方法，异步提交红心状态修改。
  - 对喜欢歌曲 ID 的更新进行内存状态优化（基于 `_likedSongIds.value` 在内存直接进行修改），避免不必要的网络全量重新获取开销。
  - 仅在用户位于“我喜欢的音乐”歌单且对歌曲取消喜欢时，才重新加载该歌单列表，从而实现高效的局部重构与渲染。
...
- [x] 歌单详情页更多功能半屏菜单的实际逻辑开发 (包含歌单及单曲更多选项整合)
